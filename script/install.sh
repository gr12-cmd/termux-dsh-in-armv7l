...
# 补丁：修复 node-gyp 在 Android 上的问题
patch_node_gyp() {
    log "INFO" "应用 node-gyp 补丁..."
    
    # 找到 node-gyp 的位置
    NODE_GYP_PATH=$(npm root -g)/npm/node_modules/node-gyp
    if [ -f "$NODE_GYP_PATH/lib/node-gyp.js" ]; then
        # 备份原文件
        cp "$NODE_GYP_PATH/lib/node-gyp.js" "$NODE_GYP_PATH/lib/node-gyp.js.bak"
        # 删除 OS 检测（Android 被误判）
        sed -i 's/OS="android"/OS="linux"/g' "$NODE_GYP_PATH/lib/node-gyp.js"
        log "OK" "node-gyp 补丁应用成功"
    else
        log "WARN" "未找到 node-gyp，跳过补丁"
    fi
}

# 补丁：设置 koffi 编译参数
patch_koffi() {
    log "INFO" "配置 koffi 编译参数..."
    export CFLAGS="-target armv7a-unknown-linux-androideabi30"
    export CXXFLAGS="-target armv7a-unknown-linux-androideabi30"
    log "OK" "koffi 编译参数已设置"
}

# 补丁：sharp 使用系统 libvips
patch_sharp() {
    log "INFO" "安装系统 libvips..."
    apt install -y libvips libvips-dev || log "WARN" "libvips 安装失败，sharp 可能无法使用"
